...
main_objs.push(
	maek.CPP("Tutorial-BackgroundPipeline.cpp", undefined, {
		depends: [...background_shaders],
	}),
);

//uncomment to build lines shaders and pipeline:
const lines_shaders = [maek.GLSLC("lines.vert"), maek.GLSLC("lines.frag")];
main_objs.push(
	maek.CPP("Tutorial-LinesPipeline.cpp", undefined, {
		depends: [...lines_shaders],
	}),
);
...
main_objs.push(
	maek.CPP("Tutorial-ObjectsPipeline.cpp", undefined, {
		depends: [...objects_shaders],
	}),
);
...
function custom_flags_and_rules() {
	...
	if (maek.OS === "linux") {
		VULKAN_SDK =
			...
			`${process.env.HOME}/VulkanSDK/1.4.335.0/x86_64`;
		console.log(
			`Using VULKAN_SDK='${VULKAN_SDK}'; set VULKAN_SDK environment variable to override.`,
		);
		const GLFW_DIR = process.env.GLFW_DIR || `../glfw-3.4/out`;
		console.log(
			`Using GLFW_DIR='${GLFW_DIR}'; set GLFW_DIR environment variable to override.`,
		);
		...
		VULKAN_SDK =
			process.env.VULKAN_SDK ||
			`${process.env.USERPROFILE}/VulkanSDK/1.4.335.0`;
		console.log(
			`Using VULKAN_SDK='${VULKAN_SDK}'; set VULKAN_SDK environment variable to override.`,
		);
		...
		const fs = require("fs");
		VULKAN_SDK =
			process.env.VULKAN_SDK || `${process.env.HOME}/VulkanSDK/1.4.335.1/macOS`;
		console.log(
			`Using VULKAN_SDK='${VULKAN_SDK}'; set VULKAN_SDK environment variable to override.`,
		);
	...
	maek.DEFAULT_OPTIONS.GLSLC = [
		`${VULKAN_SDK}/bin/glslc` + (maek.OS === "windows" ? ".exe" : ""),
		"-Werror",
		"-g",
		"-mfmt=c",
	...
	maek.GLSLC = (glslFile, spirvFileBase, localOptions = {}) => {
		...
		const task = async () => {
			...
			await maek.run(command, `${task.label}: compile`, async () => {
				return {
					read: [glslFile],
					written: [spirvFile],
				};
		...
		if (spirvFile in maek.tasks) {
			throw new Error(
				`Task ${task.label} purports to create ${spirvFile}, but ${maek.tasks[spirvFile].label} already creates that file.`,
			);
		}
