		...
		if (rtg.cull_mode.compare("cluster") == 0) {// Create mesh vertices from groups
			...
			for(const auto & [key, value] :rtg.meshes){
				...
				for(uint32_t idx : mesh.indices){
					...
					if(mesh.position[idx * 3] > box.maxX) box.maxX = mesh.position[idx * 3];
					if(mesh.position[idx * 3 + 1] > box.maxY) box.maxY = mesh.position[idx * 3 + 1];
					if(mesh.position[idx * 3 + 2] > box.maxZ) box.maxZ = mesh.position[idx * 3 + 2];
				}
...
void F::render(RTG &rtg_, RTG::RenderParams const &render_params) {
	...
	if (!light_instances.empty()) { //upload Lights:
		size_t needed_bytes = light_instances.size() * sizeof(ObjectsPipeline::World);
		if (workspace.World_src.handle == VK_NULL_HANDLE || workspace.World_src.size < needed_bytes) {
			// round to next multiple of 4k to avoid re-allocating continuously if vertex count grows slowly;
			size_t new_bytes = ((needed_bytes + 4096) / 4096) * 4096;
			if(workspace.World.handle) {
				rtg.helpers.destroy_buffer(std::move(workspace.World_src));
			}
			if(workspace.World.handle) {
				rtg.helpers.destroy_buffer(std::move(workspace.World));
			...
			std::cout << "Re-allocated light world buffer to " << new_bytes << " bytes." << std::endl;
		}

		assert(workspace.World_src.size == workspace.World.size);
		assert(workspace.World_src.size >= needed_bytes);

		{//copy transforms into Transforms_src:
			assert(workspace.World_src.allocation.mapped);
			ObjectsPipeline::World *out = reinterpret_cast< ObjectsPipeline::World *>(
				workspace.World_src.allocation.data()
			); //Strict aliasing violation, but it doesn't matter
		...
		vkCmdCopyBuffer(workspace.command_buffer, workspace.World_src.handle,
					workspace.World.handle, 1, &copy_region);
	}
...
void F::update(float dt) {
	...
	//Create the frustum
	{
		float tan_fov = tan(0.5f * rtg.active_camera.vfov);
		frustum.near_right = rtg.active_camera.aspect * rtg.active_camera.near * tan_fov;
        frustum.near_top = rtg.active_camera.near * tan_fov;
        frustum.near_plane = -rtg.active_camera.near;
        frustum.far_plane = -rtg.active_camera.far;
	}
	...
	for(std::string root : rtg.scene.roots){
		...
		while(!node_stack.empty()){
			...
			if(!this_node.mesh.empty()){
				...
				if(rtg.cull_mode.compare("cluster") == 0) {//cull to cluster
					for(RTG::Cluster cluster : this_mesh.clusters){
						if(in_view(mesh_box[cluster.name], CLIP_FROM_WORLD * WORLD_FROM_LOCAL)){
							object_instances.emplace_back(ObjectInstance{
								.vertices = mesh_vertices[cluster.name],
								.transform{
			...
			//Check the Light
			if(this_node.light.compare("") != 0){
				RTG::Light this_light = rtg.lights[this_node.light];
				vec4 pos = WORLD_FROM_LOCAL * vec4{0.f,0.f,0.f, 1.f};
				vec4 z = WORLD_FROM_LOCAL * vec4{0.f,0.f,1.f, 1.f};
				vec4 dir = normalize(z + (pos* -1.f));
				float strPow = 0.f;
				float angFov = 0.f;

				float type = 0.f;
				
				// std::cout << this_light.light_type << std::endl;

				if(this_light.light_type.compare("sun") == 0){
					pos = normalize(pos);
					// std::cout << pos[0] << " , " << pos[1] << " , " << pos[2] << std::endl;
					angFov = this_light.angle;
					strPow = this_light.strength;
					type = 0.f;
				} else if(this_light.light_type.compare("sphere") == 0){
					strPow = this_light.power;
					type = 1.1f;
				} else if(this_light.light_type.compare("spot") == 0){
					angFov = this_light.fov;
					strPow = this_light.power;
					type = 2.2f;
				} 
				...
				light_instances.emplace_back(Light{
					.Tint = {this_light.tint[0], this_light.tint[1], this_light.tint[2], strPow},
					.Pos = {pos[0], pos[1], pos[2], angFov},
					.RLB = {this_light.radius, this_light.limit, this_light.blend, type},
					.Dir = {dir[0], dir[1], dir[2], 0.f},
				});
			}
	...
	// {//Initialize Lights
	// 	for(const auto & [key, value] : rtg.lights){
	// 		RTG::Light l = value;
	// 		Light light;
	// 		light.Tint = {l.tint[0], l.tint[1], l.tint[2]} ;
	// 		light_instances.emplace_back(light);
	// 	}
	// }
...
bool F::in_view(AABB aabb, mat4 transform){
    ...
    corners.push_back({aabb.minX, aabb.maxY, aabb.minZ, 1.0}); // x Y z
    corners.push_back({aabb.maxX, aabb.maxY, aabb.minZ, 1.0}); // X Y z

    corners.push_back({aabb.minX, aabb.minY, aabb.maxZ, 1.0}); // x y Z
    corners.push_back({aabb.maxX, aabb.minY, aabb.maxZ, 1.0}); // X y Z
    corners.push_back({aabb.minX, aabb.maxY, aabb.maxZ, 1.0}); // x Y Z
    corners.push_back({aabb.maxX, aabb.maxY, aabb.maxZ, 1.0}); // X Y Z

	bool inside = false;
	for( vec4 corner_ : corners){
		vec4 corner = transform * corner_;
		
		inside = inside || 
			(std::abs(corner[0]) < std::abs(corner[3])) &&
			(std::abs(corner[1]) < std::abs(corner[3])) &&
			(corner[2] < std::abs(corner[3])) &&
			(0.f < corner[2]);
	}
	return inside;
}
